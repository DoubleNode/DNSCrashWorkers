...
final class WKRCrashSystemsWorkerTests: XCTestCase {
    var sut: WKRCrashSystems!
    ...
    func test_init_succeeds() {
        // Given & When
        let worker = WKRCrashSystems()

        // Then
        XCTAssertNotNil(worker)
        XCTAssertTrue(worker is WKRCrashSystems)
    }
    ...
    func test_inheritsFromBlankWorker() {
        // Given & When
        let isBlankWorker = sut is WKRBlankSystems
