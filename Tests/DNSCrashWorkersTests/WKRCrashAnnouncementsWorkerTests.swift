//
//  WKRCrashAnnouncementsWorkerTests.swift
...
final class WKRCrashAnnouncementsWorkerTests: XCTestCase {
    var sut: WKRCrashAnnouncements!

    override func setUp() {
        super.setUp()
        // Set protection level to testingSafe for safe execution
        DNSCrashWorkerProtection.protectionLevel = .testingSafe
        sut = WKRCrashAnnouncements()
    }
    ...
    func test_init_succeeds() {
        // Given & When
        let worker = WKRCrashAnnouncements()

        // Then
        XCTAssertNotNil(worker)
        XCTAssertTrue(worker is WKRCrashAnnouncements)
    }
    ...
    func test_inheritsFromBlankWorker() {
        // Given & When
        let isBlankWorker = sut is WKRBlankAnnouncements
