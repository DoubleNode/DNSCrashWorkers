...
final class WKRCrashCartWorkerTests: XCTestCase {
    var sut: WKRCrashCart!
    ...
    func test_init_succeeds() {
        // Given & When
        let worker = WKRCrashCart()

        // Then
        XCTAssertNotNil(worker)
        XCTAssertTrue(worker is WKRCrashCart)
    }
    ...
    func test_inheritsFromBlankWorker() {
        // Given & When
        let isBlankWorker = sut is WKRBlankCart
