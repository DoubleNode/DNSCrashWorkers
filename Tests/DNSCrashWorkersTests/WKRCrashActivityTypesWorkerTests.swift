//
//  WKRCrashActivityTypesWorkerTests.swift
...
final class WKRCrashActivityTypesWorkerTests: XCTestCase {
    var sut: WKRCrashActivityTypes!

    override func setUp() {
        super.setUp()
        // Set protection level to testingSafe for safe execution
        DNSCrashWorkerProtection.protectionLevel = .testingSafe
        sut = WKRCrashActivityTypes()
    }
    ...
    func test_init_succeeds() {
        // Given & When
        let worker = WKRCrashActivityTypes()

        // Then
        XCTAssertNotNil(worker)
        XCTAssertTrue(worker is WKRCrashActivityTypes)
    }
    ...
    func test_inheritsFromBlankWorker() {
        // Given & When
        let isBlankWorker = sut is WKRBlankActivityTypes
